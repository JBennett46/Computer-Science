...
void convertMove(string move, int moveArray[4], bool whiteTurn) {
        ...
        if (move[1] == 'x') {
            ...
            if (isalpha(startColChar) and isalpha(endColChar) and isdigit(endRowChar)) {
                ...
                if(startRow < 0 or startRow > 7){
                    cout << "\nInvalid Capture: Starting row out of range.\n";
                    return;
                }
    ...
    // print the start and end pos's for debugging. optional, could possibly comment out or delete now that moveArray works
...
bool checkPawns(char board[8][8], int moveArray[4]) {
    ...
    if (colDiff == 0) {
        ...
        if ((piece == 'P' and targetPiece >= 'a' and targetPiece <= 'z') or
            ...
            cout << "Invalid capture: No enemy piece to capture.\n";
            return false;
        }
    }
    else {
...
int main() { 
    ...
    while(true) {
        printBoard(board);
        cout << "\nEnter Moves (e2e4 e7e5 d2d4 dxe4 etc.): ";
        getline(cin, move);
        string singleMove = "";
